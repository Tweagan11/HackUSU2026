...
import { appReducer, initialState } from '../frontend/reducer';
import type { AppState, AppAction, UIState } from '../frontend/types';
import { PUNISHMENT_PHRASE, PUNISHMENT_REQUIRED_REPS } from '../frontend/config';

/**
 * Helper: dispatch a sequence of actions and return the final state.
 */
function reduceAll(actions: AppAction[], start: AppState = initialState): AppState {
    return actions.reduce((state, action) => appReducer(state, action), start);
}

suite('Reducer — Initial State', () => {
    test('initial state has correct defaults', () => {
        assert.strictEqual(initialState.uiState, 'BOOTING');
        assert.strictEqual(initialState.code, '');
        assert.strictEqual(initialState.challengeLanguage, '');
        assert.strictEqual(initialState.missionInstructions, '');
        assert.deepStrictEqual(initialState.dialogueLog, []);
        assert.strictEqual(initialState.resultMessage, '');
        assert.strictEqual(initialState.punishment, '');
        assert.strictEqual(initialState.punishmentPhrase, PUNISHMENT_PHRASE);
        assert.strictEqual(initialState.punishmentRequiredReps, PUNISHMENT_REQUIRED_REPS);
        assert.strictEqual(initialState.punishmentProgress, 0);
        assert.strictEqual(initialState.retryUnlocked, false);
        assert.strictEqual(initialState.attemptCount, 0);
        assert.strictEqual(initialState.callStatus, 'idle');
        assert.strictEqual(initialState.callId, '');
        assert.strictEqual(initialState.callError, '');
        assert.strictEqual(initialState.phoneNumber, '');
    });
});

suite('Reducer — Boot Sequence', () => {
    test('BOOT_COMPLETE transitions from BOOTING to BUG_ALERT', () => {
        const state = appReducer(initialState, { type: 'BOOT_COMPLETE' });
        assert.strictEqual(state.uiState, 'BUG_ALERT');
    });

    test('BOOT_COMPLETE is no-op when not in BOOTING', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'BOOT_COMPLETE' });
        assert.strictEqual(state.uiState, 'IDLE');
    });

    test('BUG_ALERT_COMPLETE transitions from BUG_ALERT to TRAINING_SPLASH', () => {
        const bugAlert: AppState = { ...initialState, uiState: 'BUG_ALERT' };
        const state = appReducer(bugAlert, { type: 'BUG_ALERT_COMPLETE' });
    ...
    test('BUG_ALERT_COMPLETE is no-op when not in BUG_ALERT', () => {
        const state = appReducer(initialState, { type: 'BUG_ALERT_COMPLETE' });
        assert.strictEqual(state.uiState, 'BOOTING');
    });

    test('TRAINING_SPLASH_COMPLETE transitions to BRIEFING_HOLD', () => {
        const splash: AppState = { ...initialState, uiState: 'TRAINING_SPLASH' };
        const state = appReducer(splash, { type: 'TRAINING_SPLASH_COMPLETE' });
    ...
    test('TRAINING_SPLASH_COMPLETE is no-op when not TRAINING_SPLASH', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'TRAINING_SPLASH_COMPLETE' });
        assert.strictEqual(state.uiState, 'IDLE');
    });
...
suite('Reducer — Challenge Loading', () => {
    test('CHALLENGE_LOADED sets code, language, and instructions', () => {
        const briefing: AppState = { ...initialState, uiState: 'BRIEFING_HOLD' };
        const state = appReducer(briefing, {
    ...
    test('CHALLENGE_LOADED is idempotent for same challenge', () => {
        ...
        const state = appReducer(existing, {
            type: 'CHALLENGE_LOADED',
            code: 'x = 1',
            language: 'python',
            instructions: 'Fix it',
...
suite('Reducer — Code Editing', () => {
    test('SET_CODE updates the code', () => {
        const state = appReducer(initialState, { type: 'SET_CODE', code: 'print("hi")' });
        assert.strictEqual(state.code, 'print("hi")');
    });

    test('SET_CODE to empty string', () => {
        const withCode: AppState = { ...initialState, code: 'something' };
        const state = appReducer(withCode, { type: 'SET_CODE', code: '' });
        assert.strictEqual(state.code, '');
    });

    test('SET_CODE preserves other state', () => {
        const withDialogue: AppState = {
            ...initialState,
            dialogueLog: [{ text: 'test', timestamp: 0 }],
            attemptCount: 3,
        };
        const state = appReducer(withDialogue, { type: 'SET_CODE', code: 'new' });
...
suite('Reducer — Submission Flow', () => {
    test('SUBMIT_CODE transitions from IDLE to ANALYZING', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'SUBMIT_CODE' });
        assert.strictEqual(state.uiState, 'ANALYZING');
        assert.ok(state.dialogueLog.length > 0, 'should add dialogue');
    });

    test('SUBMIT_CODE is no-op when not IDLE', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING' };
        const state = appReducer(analyzing, { type: 'SUBMIT_CODE' });
        assert.strictEqual(state.uiState, 'ANALYZING');
    });

    test('ANALYZE_START transitions from IDLE to ANALYZING', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'ANALYZE_START' });
        assert.strictEqual(state.uiState, 'ANALYZING');
    });

    test('ANALYZE_START from ANALYZING stays in ANALYZING (no duplicate dialogue)', () => {
        const analyzing: AppState = {
            ...initialState,
            uiState: 'ANALYZING',
            dialogueLog: [{ text: 'existing', timestamp: 0 }],
        };
        const state = appReducer(analyzing, { type: 'ANALYZE_START' });
        assert.strictEqual(state.uiState, 'ANALYZING');
        assert.strictEqual(state.dialogueLog.length, 1); // no new dialogue added
    });

    test('ANALYZE_START is no-op from RESULT_FAIL', () => {
        const fail: AppState = { ...initialState, uiState: 'RESULT_FAIL' };
        const state = appReducer(fail, { type: 'ANALYZE_START' });
...
suite('Reducer — Result FAIL', () => {
    test('RESULT_FAIL transitions from ANALYZING to RESULT_FAIL', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING' };
        const state = appReducer(analyzing, {
            type: 'RESULT_FAIL',
    ...
    test('RESULT_FAIL from IDLE also works', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'RESULT_FAIL', message: 'Timeout' });
        assert.strictEqual(state.uiState, 'RESULT_FAIL');
    });

    test('RESULT_FAIL is no-op from RESULT_PASS', () => {
        const pass: AppState = { ...initialState, uiState: 'RESULT_PASS' };
        const state = appReducer(pass, { type: 'RESULT_FAIL', message: 'late fail' });
    ...
    test('RESULT_FAIL with zero reps falls back to default (not immediate unlock)', () => {
        ...
        assert.strictEqual(state.punishmentRequiredReps, PUNISHMENT_REQUIRED_REPS);
        assert.strictEqual(state.retryUnlocked, false);
    });
    ...
    test('RESULT_FAIL with empty punishment phrase uses defaults', () => {
        ...
        assert.strictEqual(state.punishmentPhrase, PUNISHMENT_PHRASE);
    });
    ...
    test('RESULT_FAIL increments attemptCount', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING', attemptCount: 2 };
        const state = appReducer(analyzing, { type: 'RESULT_FAIL', message: 'fail' });
        assert.strictEqual(state.attemptCount, 3);
    });

    test('RESULT_FAIL adds fail-type dialogue', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING' };
        const state = appReducer(analyzing, { type: 'RESULT_FAIL', message: 'Bug found' });
        const last = state.dialogueLog[state.dialogueLog.length - 1];
...
suite('Reducer — Punishment', () => {
    test('PUNISHMENT_LINE_COMPLETED increments progress', () => {
        ...
        const state = appReducer(fail, { type: 'PUNISHMENT_LINE_COMPLETED' });
    ...
    test('PUNISHMENT_LINE_COMPLETED is no-op when not in RESULT_FAIL', () => {
        const state = appReducer(initialState, { type: 'PUNISHMENT_LINE_COMPLETED' });
        assert.strictEqual(state, initialState);
    });

    test('full punishment flow: complete all reps then retry', () => {
        let state: AppState = { ...initialState, uiState: 'ANALYZING' };

        // Fail
        state = appReducer(state, { type: 'RESULT_FAIL', message: 'bug', punishmentReps: 3 });
        assert.strictEqual(state.uiState, 'RESULT_FAIL');
        assert.strictEqual(state.retryUnlocked, false);

        // Complete 3 punishment lines
        state = appReducer(state, { type: 'PUNISHMENT_LINE_COMPLETED' });
        assert.strictEqual(state.punishmentProgress, 1);
        state = appReducer(state, { type: 'PUNISHMENT_LINE_COMPLETED' });
        ...
        // Retry
        state = appReducer(state, { type: 'RETRY' });
        assert.strictEqual(state.uiState, 'IDLE');
        assert.strictEqual(state.punishmentProgress, 0);
        assert.strictEqual(state.retryUnlocked, false);
...
suite('Reducer — Result PASS', () => {
    test('RESULT_PASS transitions from ANALYZING to RESULT_PASS', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING' };
        const state = appReducer(analyzing, { type: 'RESULT_PASS', message: 'Great job!' });
        assert.strictEqual(state.uiState, 'RESULT_PASS');
    ...
    test('RESULT_PASS is no-op when not ANALYZING', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'RESULT_PASS', message: 'pass' });
        assert.strictEqual(state.uiState, 'IDLE');
    });

    test('RESULT_PASS adds dialogue', () => {
        const analyzing: AppState = { ...initialState, uiState: 'ANALYZING' };
        const state = appReducer(analyzing, { type: 'RESULT_PASS', message: 'Done' });
...
suite('Reducer — Retry', () => {
    test('RETRY transitions from RESULT_FAIL to IDLE when unlocked', () => {
        ...
        const state = appReducer(fail, { type: 'RETRY' });
        assert.strictEqual(state.uiState, 'IDLE');
        assert.strictEqual(state.punishment, '');
    ...
    test('RETRY is no-op when not in RESULT_FAIL', () => {
        const idle: AppState = { ...initialState, uiState: 'IDLE' };
        const state = appReducer(idle, { type: 'RETRY' });
        assert.strictEqual(state.uiState, 'IDLE');
    });
...
suite('Reducer — Phone Call Flow', () => {
    ...
    test('CALL_REQUESTED sets status to requested', () => {
        const state = appReducer(initialState, { type: 'CALL_REQUESTED' });
    ...
    test('CALL_INITIATED sets status to calling with callId', () => {
        const requested: AppState = { ...initialState, callStatus: 'requested' };
        const state = appReducer(requested, { type: 'CALL_INITIATED', callId: 'call-123' });
    ...
    test('CALL_IN_PROGRESS sets status to in-progress', () => {
        const calling: AppState = { ...initialState, callStatus: 'calling' };
        const state = appReducer(calling, { type: 'CALL_IN_PROGRESS' });
        assert.strictEqual(state.callStatus, 'in-progress');
    });

    test('CALL_ENDED sets status to ended', () => {
        const inProgress: AppState = { ...initialState, callStatus: 'in-progress' };
        const state = appReducer(inProgress, { type: 'CALL_ENDED' });
        assert.strictEqual(state.callStatus, 'ended');
    });

    test('CALL_ERROR sets status to error with message', () => {
        const calling: AppState = { ...initialState, callStatus: 'calling' };
        const state = appReducer(calling, { type: 'CALL_ERROR', message: 'Connection failed' });
    ...
    test('CALL_DISMISSED resets call state', () => {
        ...
        const state = appReducer(ended, { type: 'CALL_DISMISSED' });
    ...
    test('full call flow: requested → initiated → in-progress → ended → dismissed', () => {
        let state = appReducer(initialState, { type: 'CALL_REQUESTED' });
        assert.strictEqual(state.callStatus, 'requested');

        state = appReducer(state, { type: 'CALL_INITIATED', callId: 'call-999' });
        assert.strictEqual(state.callStatus, 'calling');

        state = appReducer(state, { type: 'CALL_IN_PROGRESS' });
        assert.strictEqual(state.callStatus, 'in-progress');

        state = appReducer(state, { type: 'CALL_ENDED' });
        assert.strictEqual(state.callStatus, 'ended');

        state = appReducer(state, { type: 'CALL_DISMISSED' });
        assert.strictEqual(state.callStatus, 'idle');
    });

    test('call error flow: requested → initiated → error → dismissed', () => {
        let state = appReducer(initialState, { type: 'CALL_REQUESTED' });
        state = appReducer(state, { type: 'CALL_INITIATED', callId: 'call-err' });
        state = appReducer(state, { type: 'CALL_ERROR', message: 'Twilio down' });
        assert.strictEqual(state.callStatus, 'error');
...
suite('Reducer — Full Integration Scenarios', () => {
    ...
    test('multiple failures accumulate attemptCount', () => {
        let state: AppState = { ...initialState, uiState: 'IDLE' };
        for (let i = 0; i < 5; i++) {
            state = appReducer(state, { type: 'SUBMIT_CODE' });
            state = appReducer(state, { type: 'RESULT_FAIL', message: `fail ${i}`, punishmentReps: 2 });
    ...
    test('unknown action type is ignored (returns same state)', () => {
        const state = appReducer(initialState, { type: 'NONEXISTENT' } as unknown as AppAction);
        assert.strictEqual(state, initialState);
    });
});
