...
function parsePersistedPanelState(value) {
    ...
    if (candidate.version !== 1)
        return null;
    if (typeof candidate.appState !== 'object' || candidate.appState === null)
        return null;
...
suite('Extension — stripLeadingEnvAssignments', () => {
    ...
    test('multiple env assignments stripped', () => {
        assert.strictEqual(stripLeadingEnvAssignments('A=1 B=2 C=3 node app.js'), 'node app.js');
    });
    test('env with double-quoted value', () => {
        assert.strictEqual(stripLeadingEnvAssignments('MSG="hello world" python test.py'), 'python test.py');
    });
    test('env with single-quoted value', () => {
        assert.strictEqual(stripLeadingEnvAssignments("PATH='/usr/bin' python test.py"), 'python test.py');
    });
    test('empty input', () => {
...
suite('Extension — parsePersistedPanelState', () => {
    ...
    test('returns null for wrong version', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, version: 2 }), null);
    });
    ...
    test('returns null for null appState', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, appState: null }), null);
    });
    test('returns null for non-object appState', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, appState: 'string' }), null);
    });
    test('returns null for NaN timeLeftSec', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, timeLeftSec: NaN }), null);
    });
    test('returns null for Infinity timeLeftSec', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, timeLeftSec: Infinity }), null);
    });
    test('returns null for string timeLeftSec', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, timeLeftSec: '60' }), null);
    });
    test('returns null for NaN savedAt', () => {
        assert.strictEqual(parsePersistedPanelState({ ...validState, savedAt: NaN }), null);
    });
...
suite('Extension — Integration Scenarios', () => {
    ...
    test('non-triggering commands do not activate', () => {
        ...
        for (const cmd of nonTriggers) {
            assert.strictEqual(shouldAutoTriggerForCommand(cmd), false, `"${cmd}" should NOT trigger`);
        }
    });
    test('all triggering commands activate correctly', () => {
        ...
        for (const cmd of triggers) {
            assert.strictEqual(shouldAutoTriggerForCommand(cmd), true, `"${cmd}" should trigger`);
        }
    });
});
//# sourceMappingURL=extension-helpers.test.js.map