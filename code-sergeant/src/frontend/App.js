...
const BriefingHold_1 = __importDefault(require("./components/BriefingHold"));
...
const SuspicionAlert_1 = __importDefault(require("./components/SuspicionAlert"));
...
const App = () => {
    ...
    const [state, dispatch] = (0, react_1.useReducer)(reducer_1.appReducer, {
        ...
        punishmentPhrase: persistedState?.appState.punishmentPhrase ?? config_1.PUNISHMENT_PHRASE,
        punishmentRequiredReps: persistedState?.appState.punishmentRequiredReps ?? config_1.PUNISHMENT_REQUIRED_REPS,
    ...
    (0, react_1.useEffect)(() => {
        if (state.uiState !== 'BUG_ALERT')
            return;
        const timer = setTimeout(() => dispatch({ type: 'BUG_ALERT_COMPLETE' }), config_1.BUG_ALERT_DURATION_MS);
        return () => clearTimeout(timer);
    }, [state.uiState]);
    ...
    // --- Handlers ---
    const sendCodeForAnalysis = (0, react_1.useCallback)(() => {
        if (isDevMode) {
    ...
    const handleResubmit = (0, react_1.useCallback)(() => {
        if (state.uiState !== 'RESULT_FAIL' || !state.retryUnlocked)
            return;
        dispatch({ type: 'RETRY' });
        dispatch({ type: 'SUBMIT_CODE' });
        sendCodeForAnalysis();
    ...
    const isEditorReadOnly = state.uiState === 'BOOTING' ||
    ...
    const rootClasses = [
        ...
        .filter(Boolean)
        .join(' ');
    return ((0, jsx_runtime_1.jsxs)("div", { className: rootClasses, children: [state.uiState === 'BOOTING' && (0, jsx_runtime_1.jsx)(BootScreen_1.default, {}), state.uiState === 'BUG_ALERT' && (0, jsx_runtime_1.jsx)(SuspicionAlert_1.default, {}), state.uiState === 'TRAINING_SPLASH' && (0, jsx_runtime_1.jsx)(TrainingSplash_1.default, {}), state.uiState === 'BRIEFING_HOLD' && (0, jsx_runtime_1.jsx)(BriefingHold_1.default, {}), state.uiState !== 'BOOTING' &&
                state.uiState !== 'BUG_ALERT' &&
                state.uiState !== 'TRAINING_SPLASH' &&
                state.uiState !== 'BRIEFING_HOLD' && ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)(TopBar_1.default, { uiState: state.uiState, mood: mood, timeLeftSec: timeLeftSec }), (0, jsx_runtime_1.jsx)(MissionLayout_1.default, { code: state.code, onCodeChange: handleCodeChange, editorLanguage: resolvedLanguage, readOnly: isEditorReadOnly, uiState: state.uiState, missionInstructions: state.missionInstructions, dialogueLog: state.dialogueLog, punishment: state.punishment, punishmentPhrase: state.punishmentPhrase, punishmentRequiredReps: state.punishmentRequiredReps, punishmentProgress: state.punishmentProgress, showPunishment: state.uiState === 'RESULT_FAIL', onPunishmentLineCompleted: handlePunishmentLineCompleted, onSubmit: handleSubmit, onResubmit: handleResubmit, onNextMission: handleNextMission, canRetryAfterPunishment: state.uiState === 'RESULT_FAIL' ? state.retryUnlocked : true })] })), state.uiState === 'ANALYZING' && (0, jsx_runtime_1.jsx)(AnalyzingOverlay_1.default, {}), (state.uiState === 'RESULT_PASS' ||
